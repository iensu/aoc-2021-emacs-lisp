...
(let* ((input (aoc-utils/read-input "../inputs/03.txt"))
       (threshold (/ (length input) 2))
       (bits-per-input (length (car input)))
       (max-input (- (expt 2 bits-per-input) 1))
       (bits-by-position (apply (apply-partially #'cl-mapcar #'list)
                                (cl-mapcar #'string-to-list input)))
       (gamma (string-to-number (mapconcat (lambda (bits) (if (> (count ?1 bits) threshold)
                                                              "1"
                                                            "0"))
                                                bits-by-position)
                                     2))
...
;;   than 1 bits (7), so keep only the 5 numbers with a 0 in the first position: 00100, 01111, 00111, 00010, and
;;   01010.
